...
async function verifyContract(
  ...
  try {
    ...
    if (msg.toLowerCase().includes('already verified')) {
      ...
      console.log(
        `  ℹ️   ConfluxScan constructor_args mismatch (viaIR limitation) — check Sourcify for ✅`
      );
    } else {
...
async function main() {
  ...
  if (networkName === 'hardhat') {
    console.error(
      '❌  Cannot verify on the in-memory Hardhat network. Use --network espaceTestnet or --network espaceMainnet.'
    );
    process.exit(1);
  }
  ...
  if (missing.length > 0) {
    ...
    console.error(
      '    Run deploy.ts first and copy the printed .env snippet.\n'
    );
    process.exit(1);
  }
  ...
  await verifyContract('SwappiPriceAdapter', priceAdapter!, [
    router,
    factory,
    deployer.address,
  ]);

  await verifyContract('AutomationManager', automationManager!, [
    priceAdapter!,
    deployer.address,
  ]);

  await verifyContract('PermitHandler', permitHandler!, []);
