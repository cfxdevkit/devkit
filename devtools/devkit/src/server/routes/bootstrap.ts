...
export function createBootstrapRoutes(nodeManager: NodeManager): Router {
  ...
  router.get('/catalog', (_req, res) => {
    ...
    const precompiles = Object.values(PRECOMPILES).map(
      ({ name, category, description, chains, address }) => ({
        type: 'precompile' as const,
        name,
        category,
        description,
        chains,
    ...
    res.json([...deployable, ...precompiles]);
  });
  ...
  router.get('/catalog/:name', (req, res) => {
    ...
    if (entry) {
      res.json({ type: 'deployable', ...entry });
      return;
    }

    const precompile = PRECOMPILES[name];
    if (precompile) {
      res.json({ type: 'precompile', ...precompile });
      return;
    }
  ...
  router.post('/deploy', async (req, res) => {
    ...
    if (!entry) {
      res.status(404).json({
        error: `Catalog entry "${name}" not found or is a precompile`,
      });
      return;
    }
    ...
    if (!entry.bytecode) {
      res.status(400).json({
        error: `Entry "${name}" has no bytecode — it is a precompile`,
      });
      return;
    }
    ...
    if (!entry.chains.includes(chain)) {
      res
        .status(400)
        .json({ error: `"${name}" does not support chain "${chain}"` });
      return;
    }
    ...
    try {
      ...
      const status = msg.includes('Account index')
        ? 400
